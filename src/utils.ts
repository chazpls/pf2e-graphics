...
export function devLog(...args: any) {
	const devEnabled = dev || window.pf2eGraphics?.liveSettings?.dev;
	if (devEnabled) console.log(`[%cPF2e Graphics%c %cDEV%c]`, 'color: yellow', '', 'color: #20C20E;', '', ...args);
}

/** Fires `console.log()` with some formatting sugar TODO: NOT if and only if `dev` mode is enabled. */
export function log(...args: any) {
	const devEnabled = dev || window.pf2eGraphics?.liveSettings?.dev;
	if (devEnabled) console.log(`[%cPF2e Graphics%c]`, 'color: yellow', '', ...args);
}
