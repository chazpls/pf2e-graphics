...
export let AnimCore = class AnimCore {
	...
	search(
		...
		for (const [rollOption, animations] of animationData.entries()) {
			...
			devLog(`Matched animation key: ${rollOption}`);
		...
		function filterByPredicates<T extends { predicates?: PredicateStatement[] }>(animations: T[]) {
			return animations.filter(animation => {
				const passes = game.pf2e.Predicate.test(animation.predicates, rollOptions);
				if (!passes && animation.predicates && animation.predicates.length > 0) {
					devLog(`Animation filtered out by predicates:`, animation.predicates);
				}
				return passes;
			});
		}
