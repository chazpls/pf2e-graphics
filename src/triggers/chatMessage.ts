...
function handleChatMessage(message: ChatMessagePF2e, delayed = false) {
	...
	// Add target:distance options for v13 compatibility
	const distanceOptions: string[] = [];
	if (sources.length > 0 && targets.length > 0) {
		for (const target of targets.filter(nonNullable)) {
			try {
				const sourceToken = sources[0].object ?? sources[0];
				const targetToken = target.object ?? target;
				log('Distance calc - sourceToken:', sourceToken, 'targetToken:', targetToken);
				if (sourceToken && targetToken && canvas.grid) {
					...
					distanceOptions.push(`target:distance:${Math.floor(distance)}`);
					distanceOptions.push(`target:distance`);
				}
			} catch (e) {
	...
	log('Distance options to add:', distanceOptions);

	window.pf2eGraphics.AnimCore.animate(
		{
			rollOptions: rollOptions.concat(newOptions).concat(distanceOptions),
			trigger,
			context: message,
			sources,
